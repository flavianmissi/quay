...
class TestProxy(unittest.TestCase):
    ...
    def test_get_manifest_404(self):
        ...
        msg = f"GET {proxy.base_url}/v2/library/postgres/manifests/{TAG_404}: "
    ...
    def test_session_request_wrapper_retries_request_on_failed_auth(self):
        ...
        proxy._authorize = auth_mock
        proxy._request(
            get_mock,
            manifests_url,
            headers=manifests_headers,
        )
        manifest_call = mock.call(
            manifests_url,
            headers=manifests_headers,
        )
        expected_calls = [
            manifest_call,  # token expired - returns 401
            manifest_call,
        ]
        self.assertEqual(expected_calls, get_mock.mock_calls)
        self.assertEqual(auth_mock.call_count, 1)

    def test_manifest_exists(self):
        with HTTMock(docker_registry_mock):
            proxy = Proxy(self.config, "library/postgres")
            resp = proxy.manifest_exists(image_ref=TAG)
    ...
    def test_manifest_exists_404(self):
        with HTTMock(docker_registry_mock):
            proxy = Proxy(self.config, "library/postgres")
            resp = proxy.manifest_exists(image_ref=TAG_404)
    ...
    def test_get_blob(self):
        with HTTMock(docker_registry_mock):
            proxy = Proxy(self.config, "library/postgres")
            resp = proxy.get_blob(
                digest=DIGEST, media_type="application/vnd.docker.distribution.manifest.v2+json"
            )
        self.assertEqual(resp["status"], 200)
    ...
    def test_get_blob_404(self):
        ...
        msg = f"GET {proxy.base_url}/v2/library/postgres/blobs/{DIGEST_404}: "
        ...
        error = json.loads(resp["response"])["errors"][0]
        self.assertEqual(unknown_blob, error)
    ...
    def test_blob_exists_404(self):
        with HTTMock(docker_registry_mock):
            proxy = Proxy(self.config, "library/postgres")
            resp = proxy.blob_exists(digest=DIGEST_404)
