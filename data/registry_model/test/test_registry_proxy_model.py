...
from data.model import oci, ManifestDoesNotExist
...
@pytest.fixture
def create_manifest():
    def _create_manifest(repo_id: int, manifest: str, media_type: str):
        parsed = parse_manifest_from_bytes(
            Bytes.for_string_or_unicode(manifest),
            media_type,
            sparse_manifest_support=True,
        )
        m = oci.manifest.create_manifest(repo_id, parsed)
...
class TestRegistryProxyModelCreateManifestAndRetargetTag:
    ...
    def test_create_sub_manifests_for_manifest_list(self, create_repo):
        ...
        expected_count = len(list(input_manifest.child_manifests()))
...
class TestRegistryProxyModelLookupManifestByDigest:
    ...
    def test_returns_cached_manifest_when_it_exists_upstream(self, create_repo):
        ...
        proxy_mock.manifest_exists.return_value = {
            "status": 200,
            "headers": {"docker-content-digest": UBI8_8_4_DIGEST},
        }
        proxy_mock.get_manifest.return_value = {
            "status": 200,
            "response": UBI8_8_4_MANIFEST_SCHEMA2,
    ...
    def test_renew_tag_when_manifest_is_cached_and_exists_upstream(self, create_repo):
        repo_ref = create_repo(self.orgname, self.upstream_repository, self.user)
        proxy_mock = MagicMock()
        proxy_mock.manifest_exists.return_value = {
            "status": 200,
        ...
        with patch(
            ...
            manifest = proxy_model.lookup_manifest_by_digest(repo_ref, UBI8_8_4_DIGEST)
        assert manifest is not None
        first_tag = oci.tag.get_tag_by_manifest_id(manifest.id)
        ...
        with patch(
            ...
            manifest = proxy_model.lookup_manifest_by_digest(repo_ref, UBI8_8_4_DIGEST)
        assert manifest is not None
        tag = oci.tag.get_tag_by_manifest_id(manifest.id)
    ...
    def test_renew_manifest_and_parent_tag_when_manifest_is_child_of_manifest_list(
        ...
        proxy_mock.manifest_exists.return_value = {
            "status": 200,
            "headers": {"docker-content-digest": UBI8_LATEST_DIGEST},
        }
        proxy_mock.get_manifest.return_value = {
            "status": 200,
            "response": UBI8_8_4_MANIFEST_SCHEMA2,
        ...
        updated_tag = oci.tag.get_tag_by_manifest_id(manifest.id)
        updated_list_tag = oci.tag.get_tag_by_manifest_id(manifest_list.id)
    ...
    def test_returns_cached_manifest_when_it_was_deleted_upstream_and_cache_is_not_expired(
        self, create_repo
    ):
        repo_ref = create_repo(self.orgname, self.upstream_repository, self.user)
        proxy_mock = MagicMock()
    ...
    def test_does_not_bump_tag_expiration_when_manifest_is_cached_and_upstream_is_down(
        self, create_repo
    ):
        repo_ref = create_repo(self.orgname, self.upstream_repository, self.user)
        proxy_mock = MagicMock()
        ...
        proxy_mock = MagicMock()
        proxy_mock.manifest_exists.return_value = {"status": 500}
        proxy_mock.get_manifest.return_value = {"status": 500}
        with patch(
            "data.registry_model.registry_proxy_model.Proxy", MagicMock(return_value=proxy_mock)
        ):
            proxy_model = ProxyModel(
...
class TestRegistryProxyModelGetRepoTag:
    ...
    def test_updates_manifest_and_bumps_tag_expiration_when_upstream_manifest_changed(
        ...
        proxy_mock.manifest_exists.return_value = {
            "status": 200,
            "headers": {"docker-content-digest": UBI8_8_5_DIGEST},
        }
        proxy_mock.get_manifest.return_value = {
            "status": 200,
            "response": UBI8_8_5_MANIFEST_SCHEMA2,
            "headers": {"content-type": DOCKER_SCHEMA2_MANIFEST_CONTENT_TYPE},
        }
    ...
    def test_passes_through_upstream_error_when_image_isnt_cached(self, create_repo):
        ...
        with patch(
            ...
            with pytest.raises(ManifestDoesNotExist):
                proxy_model.get_repo_tag(repo_ref, self.tag)

    def test_passes_through_upstream_error_when_local_cache_is_expired(self, create_repo):
        repo_ref = create_repo(self.orgname, self.upstream_repository, self.user)
        proxy_mock = MagicMock()
        proxy_mock.manifest_exists.return_value = {
            "status": 200,
    ...
    def test_doesnt_bump_tag_expiration_when_upstream_is_dead(self, create_repo):
        repo_ref = create_repo(self.orgname, self.upstream_repository, self.user)
        proxy_mock = MagicMock()
        proxy_mock.manifest_exists.return_value = {
            "status": 200,
