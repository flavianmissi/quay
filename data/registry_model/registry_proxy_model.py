...
from data.model import oci, RepositoryDoesNotExist, ManifestDoesNotExist
...
class UpstreamRegistryError(Exception):
    def __init__(self, status_code):
        msg = f"Error requesting upstream registry: {status_code}"
        # msg = f"manifest not found upstream: {resp['status']}"
        super().__init__(msg)
...
class ProxyModel(OCIModel):
    ...
    def lookup_repository(self, namespace_name, repo_name, kind_filter=None, raise_on_error=False):
        ...
        NOTE: repository creation depends on self._manifest_ref pointing to a
        valid upstream manifest tag or digest. If called in the context of a blob request,
        self._manifest_ref will not be a valid manifest reference, and this method will fail
        to create a repository, returning None.
        """
        ...
        resp = self._proxy.manifest_exists(self._manifest_ref)
        upstream_ok = resp["status"] == 200
        if not upstream_ok:
            msg = f"failed to get manifest from upstream registry: {resp['status']}"
            if raise_on_error:
                raise RepositoryDoesNotExist(msg)
            return None
    ...
    def lookup_manifest_by_digest(
        ...
        db_tag = oci.tag.get_tag_by_manifest_id(wrapped_manifest.id)
        # TODO: will this tag ever be None?
        ...
        try:
            ...
            return wrapped_manifest if not existing_tag.expired else None
            ...
            with db_disallow_replica_use():
                ...
                try:
                    ...
                    parent_tag = (
                        TagTable.select()
                        .where(
                            TagTable.repository_id == repository_ref.id, TagTable.manifest == parent
                        )
                        .order_by(-TagTable.lifetime_end_ms)
                        .get()
                    )
    ...
    def get_repo_tag(self, repository_ref, tag_name, raise_on_error=True):
        ...
        """
        try:
            # TODO: test repository with multiple tags with the same name
            db_tag = (
                TagTable.select()
                .where(TagTable.repository == repository_ref.id)
        ...
        try:
            ...
            # expiration here either - we only do this when we can ensure
            # the tag exists upstream.
            return existing_tag if not existing_tag.expired else None
    ...
    def _create_and_tag_manifest(
        self, repo_ref: RepositoryReference, manifest_ref: str, create_manifest_fn
    ) -> tuple[Manifest, Tag]:
        """
        ...
        """
        manifest_exists = self._proxy.manifest_exists(manifest_ref, self._media_type)
        upstream_ok = manifest_exists["status"] == 200
        if not upstream_ok:
            # msg = f"manifest not found upstream: {resp['status']}"
            raise UpstreamRegistryError(status_code=manifest_exists["status"])

        manifest = self._pull_upstream_manifest(repo_ref.name, manifest_ref, self._media_type)

        manifest, tag = create_manifest_fn(repo_ref, manifest, manifest_ref)
        return manifest, tag

    def _update_manifest_for_tag(
        self, repo_ref: RepositoryReference, tag: Tag, manifest: Manifest, create_manifest_fn
    ) -> (Tag, bool):
        """
        ...
        the upstream registry. A manifest is a placeholder when its db entry
        exists, but the manifest_bytes field is empty.
        ...
        """
        manifest_exists = self._proxy.manifest_exists(tag.name, self._media_type)
        upstream_ok = manifest_exists["status"] == 200
        if not upstream_ok:
            raise UpstreamRegistryError(status_code=manifest_exists["status"])

        upstream_digest = manifest_exists["headers"]["docker-content-digest"]
        ...
        upstream_manifest = self._pull_upstream_manifest(repo_ref.name, tag.name, self._media_type)
        if up_to_date and placeholder:
            with db_disallow_replica_use():
                with db_transaction():
                    ...
                    db_tag = oci.tag.get_tag_by_manifest_id(manifest.id)
        ...
        _, tag = create_manifest_fn(repo_ref, upstream_manifest, tag.name)
        return tag, True

    def _create_manifest_and_retarget_tag(
        self, repository_ref, manifest, tag_name
    ...
    def _create_manifest_with_temp_tag(self, repository_ref, manifest, manifest_ref=None):
        with db_disallow_replica_use():
            with db_transaction():
                ...
                expiration = self._config.expiration_s or None
                tag = oci.tag.create_temporary_tag_if_necessary(db_manifest, expiration)
                wrapped_manifest = Manifest.for_manifest(db_manifest, self._legacy_image_id_handler)
    ...
    def blob_exists(self, model_cache, namespace_name, repo_name, blob_digest):
        ...
        Downloads the blob from upstream if the blob exists, but is a placeholder.
        Caches the blob if found.
        """
        # deliberately gets repo with parent lookup_repository method - we don't want
        # to create the repo here if it doesn't already exist. the OCI distribution spec
        # pull flow specifies that the manifest is the first thing to get downloaded
        # during a pull, and that's where we create the repository, so we expect it to
        # exist here.
        repo_ref = super().lookup_repository(namespace_name, repo_name)
        if repo_ref is None:
            return False

        # now that we know the repository exists we can defer to get_cached_repo_blob.
    ...
    def get_repo_blob_by_digest(self, repository_ref, blob_digest, include_placements=False):
        ...
        with db_disallow_replica_use():
            with db_transaction():
                try:
                    ImageStoragePlacement.filter(storage=blob).get()
                except ImageStoragePlacement.DoesNotExist:
                    exists_upstream = self._proxy.blob_exists(blob_digest)["status"] == 200
                    ...
                    # TODO: should we use range requests to download the blob when
                    # the server supports it?
                    storage.put_content([location_name], path, resp["response"])
    ...
    def convert_manifest(
        ...
        if parsed.media_type in allowed_mediatypes:
            return parsed

        schema1 = self.get_schema1_parsed_manifest(
    ...
    def get_schema1_parsed_manifest(self, manifest, namespace_name, repo_name, tag_name, storage):
        ...
        if parsed.is_manifest_list:
            exists_upstream = self._proxy.manifest_exists(parsed.digest)["status"] == 200
            if not exists_upstream:
                return None

            linux_amd64_digest = parsed.amd64_linux_manifest_digest
            try:
                parsed = self._pull_upstream_manifest(
                    f"{namespace_name}/{repo_name}",
                    linux_amd64_digest,
                    DOCKER_SCHEMA1_MANIFEST_CONTENT_TYPE,
                )
        ...
        if not parsed.is_image_manifest:
            return None
            ...
            with db_disallow_replica_use():
                with db_transaction():
                    try:
                        self._create_manifest_and_retarget_tag(repo_ref, parsed, tag_name)
                    except (oci.manifest.CreateManifestException, oci.tag.RetargetTagException):
                        return None
                    except oci.manifest._ManifestAlreadyExists:
                        pass

        if parsed.schema_version == 1:
            return parsed

        # NOTE: uses the upstream namespace so it matches the one in the v1 manifest
        upstream_ns = self._upstream_namespace(f"{namespace_name}/{repo_name}")
        v1_builder = DockerSchema1ManifestBuilder(upstream_ns, repo_name, tag_name)

        for layer in parsed.filesystem_layers:
            v1_builder.add_layer(str(layer.digest), None)

        schema1 = v1_builder.build()
        return schema1
    ...
    def _pull_upstream_manifest(
        self, repo: str, manifest_ref: str, media_type: str
    ) -> ManifestInterface | None:
        resp = self._proxy.get_manifest(manifest_ref, media_type)
        if resp["status"] != 200:
            msg = f"manifest not found upstream: {resp['status']}"
            raise ManifestDoesNotExist(msg)
