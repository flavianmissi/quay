...
class TestManifestPullThroughStorage:
    ...
    def test_creates_repo_on_first_pull(self, test_name):
        ...
        proxy_mock.manifest_exists.return_value = {
            "status": 200,
            "headers": {"docker-content-digest": "ab12f"},
        }
        proxy_mock.get_manifest.return_value = {
            "status": 200,
            "response": test_params["manifest_json"],
            "headers": {"content-type": test_params["manifest_type"]},
        }
        ...
        with patch(
            ...
            headers["Accept"] = ", ".join(DOCKER_SCHEMA2_CONTENT_TYPES.union(OCI_CONTENT_TYPES))
            if test_params["manifest_type"] == DOCKER_SCHEMA1_MANIFEST_CONTENT_TYPE:
                headers["Accept"] = DOCKER_SCHEMA1_MANIFEST_CONTENT_TYPE
    ...
    def test_does_not_create_repo_when_upstream_repo_does_not_exist(self, test_name):
        test_params = storage_test_cases[test_name]
        repo = f"{self.orgname}/{test_params['image_name']}"
        params = {
            "repository": repo,
    ...
    def test_does_not_create_repo_when_already_exists(self, test_name):
        test_params = storage_test_cases[test_name]
        repo = f"{self.orgname}/{test_params['image_name']}"
        params = {
            "repository": repo,
    ...
    def test_does_not_pull_from_upstream_when_manifest_already_exists(self, test_name):
        test_params = storage_test_cases[test_name]
        repo = f"{self.orgname}/{test_params['image_name']}"
        params = {
            "repository": repo,
        ...
        proxy_mock.manifest_exists.return_value = {
            "status": 200,
            "headers": {"docker-content-digest": manifest.digest},
        }
        proxy_mock.get_manifest.return_value = {
            "status": 200,
            "response": test_params["manifest_json"],
    ...
    def test_create_manifest_config_blob(self, test_name):
        ...
        proxy_mock.get_blob.return_value = {
            "status": 200,
            "response": HELLO_WORLD_CONFIG_BLOB,
            "headers": {"content-type": DOCKER_SCHEMA2_CONFIG_CONTENT_TYPE},
        }
        ...
        with patch(
            ...
            resp = conduct_call(
                ...
                params,
                expected_code=200,
                headers=headers,  # empty Accept, so we get a schema1
            )
    ...
    def test_pull_schema1_manifest_when_client_doesnt_support_v2_manifests(self, test_name):
        test_params = storage_test_cases[test_name]
        if test_params["manifest_type"] in [
            DOCKER_SCHEMA2_MANIFESTLIST_CONTENT_TYPE,
            OCI_IMAGE_INDEX_CONTENT_TYPE,
        ]:
            pytest.skip("manifest list detected, skipping manifest specific test.")

        if test_params["ref_type"] == "digest":
            pytest.skip(
                "skipping fetch by digest - it will return the original manifest's media type"
            )
        ...
        output_manifest = parse_manifest_from_bytes(
            Bytes.for_string_or_unicode(resp.data),
            resp.mimetype,
            sparse_manifest_support=True,
        )

        assert output_manifest.schema_version == 1
        assert output_manifest.namespace == "library"
        assert output_manifest.repo_name == test_params["image_name"]
        assert output_manifest.tag == tag.name

    def test_convert_existing_v2_manifest_when_client_only_supports_v1_manifests(self, test_name):
        test_params = storage_test_cases[test_name]
        if test_params["manifest_type"] in [
            DOCKER_SCHEMA2_MANIFESTLIST_CONTENT_TYPE,
            OCI_IMAGE_INDEX_CONTENT_TYPE,
        ...
        with patch(
            ...
            resp = conduct_call(
                ...
                params,
                expected_code=200,
                headers=headers,  # NOTE: we give no Accept header, so we get a schema1 manifest
            )
        ...
        assert output_manifest.namespace == "library"
        assert output_manifest.repo_name == test_params["image_name"]
        assert output_manifest.tag == test_params["manifest_ref"]

    def test_manifest_list_create_manifest_with_sub_manifests_and_connect_them(self, test_name):
        test_params = storage_test_cases[test_name]
    ...
    def test_pull_placeholder_manifest_updates_manifest_bytes(self, test_name):
        """
        ...
        proxy_mock.get_manifest.return_value = {
            "status": 200,
            "response": HELLO_WORLD_MANIFEST_LIST_JSON,
            "headers": {"content-type": DOCKER_SCHEMA2_MANIFESTLIST_CONTENT_TYPE},
        }
        with patch(
            "data.registry_model.registry_proxy_model.Proxy", MagicMock(return_value=proxy_mock)
        ):
        ...
        proxy_mock.manifest_exists.return_value = {
            "status": 200,
            "headers": {"docker-content-digest": test_manifest.digest},
        }
        proxy_mock.get_manifest.return_value = {
            "status": 200,
            "response": test_params["manifest_json"],
